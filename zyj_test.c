...
void *thrd_func(void *arg)
{
	...
	pthread_attr_t attr;
	int state = 10;//用来接收recv和send的返回值
	
	argu.tid = pthread_self();
	argu.state = state;
	struct thrd *p;
	p = &argu;
	/*初始化线程属性对象*/
	...
	if(pthread_create(&new_thread, &attr, inspector,(void *)p) != 0)
		{
